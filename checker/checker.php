...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}
        ...
        .product-total-hint{margin-top:6px;display:inline-flex;align-items:center;gap:4px;padding:2px 12px;border-radius:999px;background:var(--secondary);color:#fff;font-size:var(--qty-font,16px);font-weight:bold;letter-spacing:.2px}
        .product-total-hint .qty-label{font-weight:600}
        .product-total-hint .qty-num{font-weight:900;line-height:1}
        body.qty-style-b .product-total-hint{background:transparent;padding:2px 0;border-radius:0;color:var(--text-soft,#6b7280);gap:6px}
        body.qty-style-b .product-total-hint .qty-label{font-weight:500}
        body.qty-style-b .product-total-hint .qty-num{min-width:1.7em;height:1.7em;padding:0 .3em;border-radius:50%;background:var(--secondary);color:#fff;font-weight:900;display:inline-flex;align-items:center;justify-content:center}
        ...
        .ct-item-qtyhint{margin-top:4px;display:inline-flex;align-items:center;gap:3px;padding:1px 8px;border-radius:999px;background:var(--secondary);color:#fff;font-size:var(--qty-font-sm,14px);font-weight:bold;letter-spacing:.2px}
        .ct-item-qtyhint .qty-label{font-weight:600}
        .ct-item-qtyhint .qty-num{font-weight:900;line-height:1}
        body.qty-style-b .ct-item-qtyhint{background:transparent;padding:1px 0;border-radius:0;color:var(--text-soft,#6b7280);gap:5px}
        body.qty-style-b .ct-item-qtyhint .qty-label{font-weight:500}
        body.qty-style-b .ct-item-qtyhint .qty-num{min-width:1.7em;height:1.7em;padding:0 .3em;border-radius:50%;background:var(--secondary);color:#fff;font-weight:900;display:inline-flex;align-items:center;justify-content:center}
...
<body>
        ...
        <div class="modal-body">
            ...
            <div id="tabPanelAppearance" class="tab-panel">
                ...
                <div class="modal-section" style="margin-top:16px">
                    ...
                    <div style="padding:6px 0 2px 4px">
                        <div class="setting-check-sub" style="margin-bottom:6px">ขนาดตัวอักษร</div>
                        <div class="appearance-btn-group">
                            <button class="appearance-btn" data-qty-font="12">S</button>
                            <button class="appearance-btn" data-qty-font="14">M</button>
                            <button class="appearance-btn" data-qty-font="16">L</button>
                            <button class="appearance-btn" data-qty-font="18">XL</button>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        const _qtyFontKey = 'checker_qty_font_' + String(currentComputerIdFromConfig || 0);
        function getQtyFont() { return parseInt(localStorage.getItem(_qtyFontKey) || '16', 10); }
        function applyQtyFont(size) {
            document.documentElement.style.setProperty('--qty-font', size + 'px');
            document.documentElement.style.setProperty('--qty-font-sm', Math.max(10, size - 2) + 'px');
            document.querySelectorAll('[data-qty-font]').forEach(function(b) {
                b.classList.toggle('active', parseInt(b.dataset.qtyFont, 10) === size);
            });
        }
        ...
        function initAppearancePanel() {
            ...
            applyQtyStyle(getQtyStyle());
            document.querySelectorAll('[data-qty-font]').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    const size = parseInt(btn.dataset.qtyFont, 10);
                    try { localStorage.setItem(_qtyFontKey, String(size)); } catch(e) {}
                    applyQtyFont(size);
                });
            });
            applyQtyFont(getQtyFont());
